...
var BoardTagORM = {
	dbName: 'ZEUS_IDEAS_BOARD_TAG',
	properties: [{
			name: "id",
			dbName: "ZEUS_IDEASBT_ID",
			id: true,
			required: true,
			type: "Long"
		},{
			name: "boardId",
			dbName: "ZEUS_IDEASBT_ZEUS_IDEASB_ID",
			id: true,
			required: true,
			type: "Long"
		},{
			name: "tagId",
			dbName: "ZEUS_IDEASBT_ANN_ID",
			id: true,
			required: true,
			type: "Long"
		}],
	associationSets: {
		board: {
			dao: require("zeus_ideas/lib/board_dao").get,
...
BoardTagDAO.prototype.listJoins = function(settings, daos){
	...
	this.$log.info('Finding '+daos.n.orm.dbName+' entities related to '+daos.m.orm.dbName+'['+boardId+']');
    ...
    try {
        ...
        this.$log.info(tagEntities.length+' '+daos.n.orm.dbName+' entities related to '+daos.m.orm.dbName+'[' + boardId+ '] found');
        return tagEntities;
    } finally {
        connection.close();
    }	
