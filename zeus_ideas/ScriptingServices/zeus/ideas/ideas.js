...
var ideasDao = require('zeus/ideas/ideasDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
		if (id) {
			var entity = ideasDao.get(id);
			if (entity !== null) {
				sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(entity, null, 2));
			} else {
				sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'idea_id\'=' + id);
			}
		} else if (count !== null) {
			var ideasCount = ideasDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', ideasCount);
		} else if (metadata !== null) {
			var ideasMetadata = ideasDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(ideasMetadata, null, 2));
		} else {
			var ideas = ideasDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(ideas, null, 2));
		}
	}
}
...
function handlePutRequest(httpRequest, httpResponse) {
	var entity = getRequestBody(httpRequest);
	var id = getIdParameter(httpRequest, xss);
	id = id !== null ? id : entity.idea_id;
	if (id !== null) {
		if (ideasDao.get(id) !== null) {
			ideasDao.update(entity);
			sendResponse(httpResponse, httpResponse.NO_CONTENT);
		} else {
			sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'idea_id\'=' + id);
		}
	} else {
		sendResponse(httpResponse, httpResponse.PRECONDITION_FAILED, 'text/plain', 'Expected \'idea_id\' parameter is missing!');
	}
...
function getIdParameter(httpRequest, xss) {
	...
	id = id !== null ? id : xss.escapeSql(httpRequest.getParameter('idea_id'));
	return id;
}
