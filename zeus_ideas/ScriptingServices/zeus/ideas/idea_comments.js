...
var idea_commentsDao = require('zeus/ideas/idea_commentsDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	var idea = xss.escapeSql(httpRequest.getParameter('idea'));
	...
	if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
		if (id) {
			var entity = idea_commentsDao.get(id);
			if (entity !== null) {
				sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(entity, null, 2));
			} else {
				sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'ideac_id\'=' + id);
			}
		} else if (count !== null) {
			var idea_commentsCount = idea_commentsDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', idea_commentsCount);
		} else if (metadata !== null) {
			var idea_commentsMetadata = idea_commentsDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(idea_commentsMetadata, null, 2));
		} else {
			var idea_comments = idea_commentsDao.list(limit, offset, sort, desc, idea);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(idea_comments, null, 2));
		}
	}
}
...
function handlePostRequest(httpRequest, httpResponse) {
	...
	var id = idea_commentsDao.create(entity);
...
function handlePutRequest(httpRequest, httpResponse) {
	var entity = getRequestBody(httpRequest);
	var id = getIdParameter(httpRequest, xss);
	id = id !== null ? id : entity.ideac_id;
	if (id !== null) {
		if (idea_commentsDao.get(id) !== null) {
			idea_commentsDao.update(entity);
			sendResponse(httpResponse, httpResponse.NO_CONTENT);
		} else {
			sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'ideac_id\'=' + id);
		}
	} else {
		sendResponse(httpResponse, httpResponse.PRECONDITION_FAILED, 'text/plain', 'Expected \'ideac_id\' parameter is missing!');
	}
...
function handleDeleteRequest(httpRequest, httpResponse, xss) {
	...
	if (id !== null) {
		var entity = idea_commentsDao.get(id);
		if (entity !== null) {
			idea_commentsDao.delete(entity);
			sendResponse(httpResponse, httpResponse.NO_CONTENT);
		} else {
			sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'ideac_id\'=' + id);
		}
...
function getIdParameter(httpRequest, xss) {
	...
	id = id !== null ? id : xss.escapeSql(httpRequest.getParameter('ideac_id'));
	return id;
}
